...
export default function About() {
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      // Center the sword vertically inside the visible sticky viewport.
      const pfadiStage = storyRef.current?.querySelector('.pfadiStage');
      if (pfadiStage && swordMotionRef.current) {
        const swordHeight = swordMotionRef.current.offsetHeight;
        const swordTop = ((stageHeight - swordHeight) / 2) - pfadiStage.offsetTop;
        swordMotionRef.current.style.top = `px`;
      }
