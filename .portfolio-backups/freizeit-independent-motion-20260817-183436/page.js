...
export default function About() {
  ...
  useEffect(() => {
    ...
    const renderStory = () => {
      ...
      const beigeY = -beigeProgress * colorTravel;
      ...
      if (beigeContentRef.current) {
        beigeContentRef.current.style.transform = `translate3d(0, ${beigeY}px, 0)`;
      }
      ...
      const bikeX = -700 + bikeProgress * 1175;
      const wheelRotation = bikeProgress * 1080;

      if (bikegroupRef.current) {
        bikegroupRef.current.style.opacity = bikeActive ? '1' : '0';
        bikegroupRef.current.style.transform = `translate3d(${bikeX}px, 0, 0)`;
      }
