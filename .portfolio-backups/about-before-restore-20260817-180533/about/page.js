...
export default function About() {
  ...
  useEffect(() => {
    ...
    const updateMeasurements = () => {
      ...
      // Beige -> dark uses one coherent 3x sweep across the visible viewport.
      // The diagonal starts as soon as its leading edge reaches the bottom of
      // the viewport and finishes when its trailing edge clears the header.
      // This makes the upper transition visibly animate again instead of only
      // accelerating for the final few pixels.
      firstCutFastStartTop = viewportHeight + panelCut;
      firstCutFastTravel = Math.max(
        1,
        (firstCutFastStartTop - headerHeight) / 3,
      );
      bikeStartTop = firstCutFastStartTop - firstCutFastTravel;

      // The bike still starts already centered/sticky when the screen becomes
      // fully dark. The negative flow offset only pre-positions the sticky
      // layer; the actual bike progress remains tied to the same scroll range.
      sceneRef.current?.style.setProperty(
        '--bike-prelift',
        `${Math.max(0, bikeStartTop - stickyTop)}px`,
      );
        ...
        * {
        ...
        .pfadiPanel {
          ...
          clip-path: polygon(
            0 0,
            100% 0,
            100% calc(100% - var(--panel-cut) - var(--cut-boost, 0px)),
        ...
        /* The upper transition is the actual clipped edge of the beige
           panel, so there is no separate horizontal rectangle edge. The
           lower transition still uses the incoming blue wedge. */
        .bikePanel::after {
          content: '';
          position: absolute;
          z-index: 2;
        ...
        .bikeStickyLayer {
          ...
          height: 520px;
          margin-top: calc(-1 * var(--bike-prelift, 0px));
          overflow: hidden;
        }
